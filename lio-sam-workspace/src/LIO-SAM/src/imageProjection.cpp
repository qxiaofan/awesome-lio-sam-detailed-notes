...
//3D视觉技术星球：https://t.zsxq.com/15TkJF4J3
...
private:
    ...
    pcl::PointCloud<PointType>::Ptr   fullCloud;            // 去畸变后的点云
    pcl::PointCloud<PointType>::Ptr   extractedCloud;       // 发送给featureExtraction节点用于特征提取，保存在cloud_info话题里面发送
...
public:
    ...
    void cloudHandler(const sensor_msgs::PointCloud2ConstPtr& laserCloudMsg)
    {
        ...
        // 进行IMU点云去畸变，并投影点云到range image上
        projectPointCloud();

        // 将点云图像中非空的点提取至输出点云中，extractedCloud用于计算曲率
        cloudExtraction();
    ...
    bool cachePointCloud(const sensor_msgs::PointCloud2ConstPtr& laserCloudMsg)
    {
        ...
        // 如果区分好了线号，就会将点云消息其中一个field的name设置为ring
    ...
    void imuDeskewInfo()
    {
        cloudInfo.imuAvailable = false; // 是否包含 IMU 校正信息
    ...
    void odomDeskewInfo()
    {
        ...
        // 其实经过上面的处理，第一个里程计消息已经在[timeScanCur-0.01，timeScanCur]的范围内了
        ...
        Eigen::Affine3f transBt = transBegin.inverse() * transEnd;  // 初始时刻到终止时刻的位姿变换
    ...
    void projectPointCloud()
    {
        ...
        for (int i = 0; i < cloudSize; ++i)
        {
            ...
            // 按到扫描开始的时间使用IMU进行点云去畸变
            // 这一步相当于论文中的 IMU为scan matching提供先验(初始猜测)
